    ...
    with open(path_to_file, 'r') as file:
        ...
        for pref in ['DUP', 'DEL', 'INV']:
            ...
            with open(os.path.join(output, name), 'r') as file:
                data = file.read()
                with open(os.path.join(output, 'new' + name), 'w') as new_file:
...
def main():
    ...
    parser.add_argument('--bedtools', nargs='?', help='path to bedtools exec', default='~/ealbert/bin/bedtools')
    ...
    for vcf in f_list:
        if vcf[-3:] == 'vcf' and not (vcf.find('DUP') != -1 or vcf.find('DEL') != -1 or vcf.find('INV') != -1):
            pool.apply_async(splitter, args=(os.path.join(args.vcfs_dir, vcf), args.vcfs_dir, args.quality))
    pool.close()
    pool.join()
    ...
    for pref in ['DUP', 'DEL', 'INV']:
        commandline = "{bedtools} unionbedg -i (vcf_dir}/*{pref}.BedGraph > {out}".format(
            vcf_dir=args.vcfs_dir,
    ...
    call('{bedtools} unionbedg -i {DUP} {DEL} {INV} > {out}'.format(
        ...
        out=os.path.join(args.output, 'cnv_combined.bed'),
        bedtools=args.bedtools), shell=True)


    table = pd.read_table(os.path.join(args.output, 'cnv_combined.bed'.format(pref)), header=None)
    selection = table.iloc[:, 0:3]
    selection['max'] = table.iloc[0:, 3:].apply(max, axis=1)
    selection.to_csv(os.path.join(args.output, 'cnv_combined.bed'), sep="\t", index=False, header=False)
